...
# Import the OrchestratorAgent (which includes Galileo tracking)
...
def initialize_agent():
    """Initialize the OrchestratorAgent (which includes Galileo tracking)"""
    try:
        if st.session_state.agent is None:
            with st.spinner("Initializing orchestrator agent..."):
                print(f"🔄 Creating new OrchestratorAgent instance...")
                st.session_state.agent = OrchestratorAgent()
                print(f"✅ OrchestratorAgent created: {id(st.session_state.agent)}")
            st.success("Orchestrator agent initialized successfully!")
        else:
            print(f"ℹ️ Reusing existing OrchestratorAgent: {id(st.session_state.agent)}")
        return True
    except ValueError as e:
        st.error(f"Failed to initialize orchestrator agent: {e}")
...
def main():
    ...
    with st.sidebar:
        ...
        # Galileo flush button
        st.markdown("---")
        st.markdown("### 🔧 Galileo Logging")
        col_flush, col_status, col_reset = st.columns([1, 1, 1])
        
        with col_flush:
            if st.button("🔄 Flush Galileo Logs", help="Manually flush Galileo traces to the system"):
                if st.session_state.agent and st.session_state.agent.galileo_evaluator:
                    success = st.session_state.agent.galileo_evaluator.flush_context()
                    if success:
                        st.success("✅ Galileo logs flushed successfully!")
                    else:
                        st.error("❌ Failed to flush Galileo logs")
                else:
                    st.warning("⚠️ No agent available to flush logs")
        
        with col_status:
            if st.session_state.agent and st.session_state.agent.galileo_evaluator:
                if st.session_state.agent.galileo_evaluator.context:
                    if st.session_state.agent.galileo_evaluator.is_context_reused():
                        st.success("✅ Galileo: Active (Reused)")
                    else:
                        st.success("✅ Galileo: Active (New)")
                else:
                    st.error("❌ Galileo: Not connected")
            else:
                st.info("ℹ️ Galileo: Not available")
        
        with col_reset:
            if st.button("🔄 Reset Context", help="Reset Galileo context (for debugging)"):
                from evaluation.galileo_evaluator import GalileoEvaluator
                GalileoEvaluator.reset_global_context()
                st.success("✅ Galileo context reset!")
                st.rerun()
        
        # Debug section
        st.markdown("### 🔍 Debug")
        col_debug, col_status = st.columns([1, 1])
        
        with col_debug:
            if st.button("🔍 Check Trace Status", help="Check if traces are being sent"):
                if st.session_state.agent and st.session_state.agent.galileo_evaluator:
                    st.session_state.agent.galileo_evaluator.check_trace_status()
                    st.info("Check console for trace status details")
                else:
                    st.warning("⚠️ No agent available")
        
        with col_status:
            if st.session_state.agent and st.session_state.agent.galileo_evaluator:
                session_id = st.session_state.agent.galileo_evaluator._session_id or "unknown"
                st.info(f"Session ID: {session_id}")
            else:
                st.info("No session ID available")
